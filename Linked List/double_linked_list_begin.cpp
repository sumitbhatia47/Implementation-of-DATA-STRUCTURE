...
{
    struct node *temp;
    temp=head; // it traverses the value from staring of the first node so that head is assigned to the temp......
    while(temp!=NULL)
    {
        cout<<temp->n1<<" "; // temp is just point to the data of traversing node......
        temp=temp->next;  // it points to the next node........
